...
Implement the public worker registration form with legacy Google Form parity, worker record creation or update, and no camp tuition payment requirement.
...
- [ ] Create or update a worker record for the camp year using the confirmed duplicate-detection strategy.
...
- [ ] Add tests for required fields, option lists, distinct task rankings, duplicate matching, worker record persistence, and informational content display.
