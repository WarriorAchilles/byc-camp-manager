...
export function FamilyRegistrationForm(): React.ReactElement {
  ...
  const continueFromGuardian = (): void => {
    setCampers((current) => current.map((camper) => ({
      ...camper,
      guardianName: camper.guardianName || guardian.fullName,
      guardianPhone: camper.guardianPhone || guardian.phone,
    })));
    setStep(2);
      ...
      const result = await apiJson<{ registrationId: string }>("/api/public/registration/family", {
        ...
        body: JSON.stringify({
          ...
          campers: campers.map((camper) => ({
            ...camper,
            address: camper.useFamilyAddress ? null : camper.address,
  ...
  if (registrationId) {
    return (
      <div className="registration-success" role="status">
        <h2>Family information saved</h2>
        <p>Your campers and signed medical authorization were saved together.</p>
  ...
  return (
    <div className="family-registration-form">
      <ol className="registration-steps" aria-label="Registration progress">
        {["Parent or guardian", "Campers", "Medical authorization"].map((label, index) => (
      ...
      {step === 1 ? (
        <form onSubmit={(event) => { event.preventDefault(); continueFromGuardian(); }}>
          <fieldset className="registration-fieldset">
            <legend>Parent or guardian information</legend>
            <div className="registration-grid">
              <label>Full name<input required autoComplete="name" value={guardian.fullName} onChange={(event) => setGuardian({ ...guardian, fullName: event.target.value })} /></label>
              ...
              <label>Relationship to camper(s)<input required value={guardian.relationship} onChange={(event) => setGuardian({ ...guardian, relationship: event.target.value })} /></label>
            </div>
          ...
          <div className="registration-actions"><button className="btn" type="submit">Continue to campers</button></div>
        </form>
      ) : null}
      ...
      {step === 2 ? (
        <form onSubmit={(event) => { event.preventDefault(); setStep(3); window.scrollTo({ top: 0, behavior: "smooth" }); }}>
          {campers.map((camper, index) => (
            <fieldset className="registration-fieldset camper-entry" key={index}>
              ...
              <div className="registration-grid registration-grid-three">
                ...
                <label>Date of birth<input required type="date" max={new Date().toISOString().slice(0, 10)} value={camper.dateOfBirth} onChange={(event) => updateCamper(index, "dateOfBirth", event.target.value)} /></label>
                ...
                <label>Parent/guardian name<input required value={camper.guardianName} onChange={(event) => updateCamper(index, "guardianName", event.target.value)} /></label>
                <label>Parent/guardian phone<input required inputMode="numeric" minLength={10} maxLength={15} value={camper.guardianPhone} onChange={(event) => updateCamper(index, "guardianPhone", digitsOnly(event.target.value))} /></label>
              ...
              {!camper.useFamilyAddress ? <AddressFields address={camper.address} options={options.stateOrProvinceOptions} onChange={(key, value) => updateCamperAddress(index, key, value)} /> : null}
              <div className="registration-grid">
          ...
          <button className="btn secondary add-camper" type="button" onClick={() => setCampers((current) => [...current, { ...emptyCamper(), guardianName: guardian.fullName, guardianPhone: guardian.phone }])}>Add another camper</button>
      ...
      {step === 3 ? (
        <form onSubmit={(event) => { event.preventDefault(); void submit(); }}>
          <fieldset className="registration-fieldset">
            ...
            <div className="agreement-copy"><p>{options.medicalAgreement.text}</p><p><strong>Covered camper(s):</strong> {campers.map((camper) => `${camper.firstName} ${camper.lastName}`).join(", ")}</p></div>
            <label className="registration-checkbox"><input required type="checkbox" checked={acknowledged} onChange={(event) => setAcknowledged(event.target.checked)} />{options.medicalAgreement.acknowledgmentText}</label>
          ...
          <div className="registration-actions"><button className="btn secondary" type="button" onClick={() => setStep(2)}>Back</button><button className="btn" type="submit" disabled={submitting}>{submitting ? "Saving…" : "Sign and save family"}</button></div>
        </form>
      ) : null}
    </div>
  );
